...
def get_prior(imnames):
    #frame1 = cv2.imread(imnames[0])
    cv2.imwrite("test.jpg",imnames[0])
    ...
    opt_name = "first_frame.jpg"
    first = np.zeros((1080, 1920))
    prior_list = []
    prior_list.append(first)
    print(bgr.shape)
    cv2.imwrite(opt_name, bgr)
    for i in range(1, len(imnames)):
        cv2.imwrite(str(i)+ ".jpg",imnames[i])
        frame2 = cv2.imread(str(i) + ".jpg")
        ...
        #imname = str(i)+".jpg"
        #print(imname)
        prior_list.append(bgr)
        #cv2.imwrite(imname, bgr)
        prvs = next
    prior = np.stack(prior_list)
    return prior
def main():
