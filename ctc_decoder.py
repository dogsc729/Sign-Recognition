...
class Decoder(object):
    ...
    def greedy_decode(self, prob, digit=False):
        ...
        #print(prob.size())# size of 5x1x32
        indexes = np.argmax(prob, axis=2)
        #print(indexes.size())
        #print(indexes)
        string = []
        prev_index = -1
    ...
    def beam_decode(self, prob, beam_size, beta=0.0, gamma=0.0, scorer=None, digit=False):
        # Performs decoding using the beam search algorithm, 
        # which maintains a set of the most probable partial hypotheses.
        # prob: [seq_len, num_labels+1], numpy array
